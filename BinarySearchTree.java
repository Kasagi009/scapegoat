...
public class BinarySearchTree<T extends Comparable<T>> implements BSTInterface<T> {
  ...
  private T minHelper(BSTNode<T> node) {
		if (node.getLeft() == null) {
			return node.getData();
		} 
    else {
 ...
 private T maxHelper(BSTNode<T> node) {
   if (node.getRight() == null) {
     return node.getData();
   } 
   else {
  ...
  public boolean equals(BSTInterface<T> other) {
	...
	private boolean equalsHelper(BSTNode<T> temp1, BSTNode<T> temp2) {
      ...
      return false;
		...
		return equalsHelper(temp1.getRight(), temp2.getRight())&& equalsHelper(temp1.getLeft(), temp2.getLeft());
	}
  ...
  public boolean sameValues(BSTInterface<T> other) {
    ...
    Iterator<T> temp1 = this.inorderIterator();
		Iterator<T> temp2 = other.inorderIterator();
		while (temp1.hasNext() && temp2.hasNext()){
			if (!(temp1.next().equals(temp2.next()))){
				return false;
      }
    }
		return !(temp1.hasNext()) && !(temp2.hasNext());
	}
  ...
  public void balance() {
    ...
    Iterator<T> iter = this.inorderIterator();
		...
		root = balanceHelper(nodes, 0, nodes.length-1);
	}

  private BSTNode<T> balanceHelper(T[] nodes, int lower, int upper) {
		if (lower > upper){
       return null;
    }
		int mid = (lower + upper)/2;
		BSTNode<T> temp = new BSTNode<T>(nodes[mid], balanceHelper(nodes, lower, mid-1), balanceHelper(nodes, mid+1, upper));
		return temp;
	}
